...
(function (global) {
  ...
  var DICT = {
    es: {
      ...
      salas_escuchar_solo_corriendo: 'Disponible cuando la sala está corriendo',
      ...
      salas_estado_error_corto: 'Error',
      salas_motivo_presupuesto: 'El worker no arrancó por el tope de cuota de desarrollo. Reiniciá el servicio de control con el código actual (ya no aplica ese tope a salas del panel).',
      salas_motivo_key: 'Falta la key de Gemini elegida en el .env del servidor.',
      salas_motivo_servicio: 'El servicio de Gemini rechazó o cortó la conexión (cupo o error interno). Reintentá en unos minutos o usá otra key.',
      salas_motivo_fuente: 'No se pudo abrir la fuente de audio (micrófono ocupado por otro programa, o archivo/stream inválido).',
      ...
      tarjetas_vacio: 'Conectando con el hub…',
      tarjetas_sin_salas: 'Todavía no hay salas en vivo. Creá una desde «Salas», arriba a la derecha.',
      ...
      estado_ctl_error: 'No arrancó',
      estado_ctl_arrancando: 'Arrancando: esperando la primera frase',
      estado_ctl_detenida: 'Detenida (configurada, sin transmitir)',
    ...
    en: {
      ...
      salas_escuchar_solo_corriendo: 'Available while the room is running',
      ...
      salas_estado_error: 'Error: {detalle}',
      salas_estado_error_corto: 'Error',
      salas_motivo_presupuesto: 'The worker did not start because of the development quota cap. Restart the control service with the current code (panel rooms no longer use that cap).',
      salas_motivo_key: 'The chosen Gemini key is missing from the server .env.',
      salas_motivo_servicio: 'The Gemini service rejected or closed the connection (quota or internal error). Retry in a few minutes or use another key.',
      salas_motivo_fuente: 'The audio source could not be opened (microphone in use by another app, or invalid file/stream).',
      ...
      tarjetas_vacio: 'Connecting to the hub…',
      tarjetas_sin_salas: 'No live rooms yet. Create one from «Rooms», top right.',
      ...
      estado_ctl_error: 'Did not start',
      estado_ctl_arrancando: 'Starting: waiting for the first sentence',
      estado_ctl_detenida: 'Stopped (configured, not broadcasting)',
